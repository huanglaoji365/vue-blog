<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="categoryList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="分类编号" align="center" prop="_id" width="120" />
      ...
      <el-table-column label="文章数量" align="center" prop="postCount" width="100" />
...
function handleUpdate(row) {
  ...
  categoriesApi.getCategory(_id).then(response => {
  ...
  proxy.$refs["categoryRef"].validate(valid => {
    if (valid) {
      if (form.value._id != null) {
        categoriesApi.updateCategory(form.value._id, form.value).then(response => {
          ElMessage.success("修改成功");
          open.value = false;
          getList();
        });
      } else {
        categoriesApi.createCategory(form.value).then(response => {
...
function handleDelete(row) {
  ...
  const categoryName = row.name || '该分类';
  
  ElMessageBox.confirm(`是否确认删除分类"${categoryName}"？`, '确认删除', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    categoriesApi.deleteCategory(_ids).then(() => {
